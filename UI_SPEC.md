...
- **Key** — the permission key string (`{service}:{action}:{arg}`; the arg may be labelled, `recipient=jane@example.com`)
...
```
┌──────────────────────────────────────────────────────┐
│  ○ Create pull request on repo overfolder/backend    │
...
Agent platforms that need i18n can build their own labels from the structured key parts (`service`, `action`, `arg`, and — when the arg is labelled — `label` + `value`) in `derived_keys`. Rendering `label` and `value` separately is what keeps a localized string from having to embed the raw `label=value` syntax.
...
1. **Service + Action** — pick one of your service instances, then pick a defined action. Parameters render as an auto-generated form (text, number, enum dropdowns, JSON textarea for object/array params). Execute hits `POST /v1/actions/call` as Mode C.
   ...
   Params the action scopes its permission keys on carry a **"scopes permission: `<label>`"** chip under the field name, from `ActionDetail.scope_param` (a `{param, label}` list). It answers "which of these values decides whether my grant covers this call?" — and, when several params share a label (`to`/`cc`/`bcc` → `recipient` on `email.send`), shows that they land in one key namespace. Unscoped params carry no chip.
...
> **Audit rows show recorded names.** Unlike every other surface, the audit table labels its actor with the name the identity had *when the event was written* (`identity_name` / `owner_user_name`, D59) — an audit row records who acted under the name they acted under. A name that has since changed is marked with a dotted underline, carries the current name in its hover, and shows a "Recorded as" line in the expanded pane; the expanded pane's SPIFFE path stays live and id-linked. Free-text search matches the recorded name, so what an operator can see is what they can search for; `identity = <name>` still resolves to an id and is unaffected by renames.
