...
- White-label token vault (`POST /v1/connections/import`, migration `082_connection_token_vault`) — partners that own their OAuth run the dance themselves and import the resulting tokens; overslash stores/refreshes/injects them and issues no `redirect_uri`. Refresh is fixed per import: self-refresh (pinned `byoc_credential_id`, hard-pinned, no cascade) or integration-managed (null BYOC → `connections.integration_managed`; injects until expiry, then `reauth_required` flagged integration-managed with no reconnect link + `connection.refresh_required` webhook; never falls back to the env/org client). Replaces and removes the per-request `redirect_uri` override + allow-list (#388/#392), the per-org `oauth_redirect_url` + `use_org_redirect` switch (#398), `POST /v1/oauth/exchange`, the `include_raw`/raw-authorize-URL surface, and `oauth_connection_flows.redirect_uri`. See [docs/design/white-label-token-vault.md](docs/design/white-label-token-vault.md).
