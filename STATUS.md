...
- **Dual subdomain surface**: `*.app.overslash.com` (browser dashboard + MCP via Vercel wildcard) and `*.api.overslash.com` (programmatic / MCP / OAuth-AS via Cloud Run + GCLB). `<slug>.app.overslash.com/mcp` is the recommended MCP endpoint — Vercel proxies it, and the OAuth handshake reuses the `.app` session cookie. `<slug>.api.overslash.com/mcp` remains fully supported for clients that want to bypass Vercel entirely. The subdomain middleware accepts either suffix and dispatches the same way; `.well-known/oauth-authorization-server` and the MCP `WWW-Authenticate` challenge return per-subdomain issuer URLs so RFC 8414 discovery works on every org subdomain, on whichever surface the client connected to.
...
- **Return-host preservation** across the apex-bound OAuth callback: when login originates on `<slug>.app.overslash.com` but the OAuth provider's pre-registered redirect_uri lands at the apex (`PUBLIC_URL` — which in cloud is `https://app.overslash.com`, itself Vercel-proxied to the API), the callback reads `oss_auth_org` + `app_host_suffix` to build an absolute redirect back to the original subdomain so users don't get stranded on the apex. Note this machinery is app-host-only by construction: it has no `api_host_suffix` branch, and the auth cookies it depends on carry `Domain=.app.overslash.com`.
...
- **Vercel**: `dashboard/vercel.json` adds wildcard host matchers for `*.app.overslash.com` (and `*.app.dev.overslash.com`), forwarding REST/auth paths to `api.overslash.com`. The OAuth-AS + MCP surface (`/.well-known/oauth-*`, `/oauth/{register,authorize,token,revoke}`, `/mcp`) is **proxied**, not redirected — the org comes from `X-Forwarded-Host`, so the issuer names the `app.*` host the client actually hit and the RFC 9728 `resource` matches the configured server URL. The file has no `redirects` at all; a redirect here would drop the `oss_session` / `oss_auth_*` cookies, which are scoped to `.app.overslash.com`. Two paths are deliberately **enumerated rather than wildcarded**: `/.well-known/*` (a wildcard would shadow `/.well-known/acme-challenge/*` and break managed-cert renewal) and `/oauth/*` (`/oauth/consent` is a real SvelteKit page and must not be proxied).
