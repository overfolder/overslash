...
async fn dev_token(
    ...
    // Step 1: ensure Dev Org exists by bootstrapping the admin profile. This
    // gives us a stable org_id to attach non-admin profiles to.
    let admin_org_id = match system.find_user_identity_by_email(admin_email).await? {
        ...
        None => match org::create(&state.db, "Dev Org", "dev-org", "standard").await {
            Ok(new_org) => {
                ...
                let admin_identity = new_scope
                    .create_identity_with_email(
                        DevProfile::Admin.display_name(),
                        "user",
                        Some(DevProfile::Admin.external_id()),
                        Some(admin_email),
                        json!({"dev": true, "profile": "admin"}),
                    )
                    .await?;
                overslash_db::repos::org_bootstrap::bootstrap_org(
                    &state.db,
                    new_org.id,
                    Some(admin_identity.id),
                )
                .await?;
                new_org.id
            }
            Err(sqlx::Error::Database(ref e)) if e.is_unique_violation() => {
                let existing = system
                    .find_user_identity_by_email(admin_email)
                    .await?
