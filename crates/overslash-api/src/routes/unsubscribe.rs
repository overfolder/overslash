...
/// HTML response after a successful (or repeat) unsubscribe via browser
/// click. Plain and self-contained so it renders identically regardless of
/// whether the apex serves the dashboard SPA.
fn confirmation_html() -> Html<&'static str> {
    Html(
        r#"<!doctype html>
<html lang="en">
<head>
...
async fn unsubscribe_get(State(state): State<AppState>, Query(q): Query<TokenQuery>) -> Response {
    ...
    match apply_unsubscribe(&state, token).await {
        Ok(true) => confirmation_html().into_response(),
        Ok(false) => (StatusCode::NOT_FOUND, "unknown token").into_response(),
        Err(()) => (
...
async fn unsubscribe_post(
    ...
    // tokens. Always 204 unless we hit a real server-side error.
    let Some(token) = q.token.as_deref().and_then(|s| Uuid::parse_str(s).ok()) else {
...
/// Shared redemption path. Returns `Ok(true)` if the token resolved to a
/// real row (whether or not it had already been redeemed — both flip the
/// user pref idempotently), `Ok(false)` if the token doesn't exist, `Err(())`
/// on any unexpected DB / mailer failure.
async fn apply_unsubscribe(state: &AppState, token: Uuid) -> Result<bool, ()> {
