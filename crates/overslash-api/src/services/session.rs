//! Shared helpers for reading the dashboard's session cookie off a
//! raw [`HeaderMap`].
//!
...
/// Decode the session cookie ([`crate::cookies::read_session`]) if present and valid. Returns `None` on
