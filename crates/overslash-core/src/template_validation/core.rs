...
use crate::types::{
    ActionParam, DeclaredRisk, McpAuth, ParamLocation, Runtime, ServiceAction, ServiceAuth,
...
fn check_sql_policy(action: &ServiceAction, action_path: &str, issues: &mut Issues) {
    let is_ident = |s: &str| {
        let mut chars = s.chars();
