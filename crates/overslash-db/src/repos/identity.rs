...
pub async fn create(
    ...
    sqlx::query_as::<_, IdentityRow>(
        "INSERT INTO identities (org_id, name, kind, external_id) VALUES ($1, $2, $3, $4)
         RETURNING id, org_id, name, kind, external_id, metadata, created_at, updated_at",
    )
    .bind(org_id)
    .bind(name)
    .bind(kind)
...
pub async fn get_by_id(pool: &PgPool, id: Uuid) -> Result<Option<IdentityRow>, sqlx::Error> {
    sqlx::query_as::<_, IdentityRow>(
        "SELECT id, org_id, name, kind, external_id, metadata, created_at, updated_at FROM identities WHERE id = $1",
    )
    .bind(id)
    .fetch_optional(pool)
    .await
...
pub async fn list_by_org(pool: &PgPool, org_id: Uuid) -> Result<Vec<IdentityRow>, sqlx::Error> {
    sqlx::query_as::<_, IdentityRow>(
        "SELECT id, org_id, name, kind, external_id, metadata, created_at, updated_at FROM identities WHERE org_id = $1 ORDER BY created_at",
    )
    .bind(org_id)
    .fetch_all(pool)
    .await
