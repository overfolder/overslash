...
- [ ] **Webhook section 7, the rest** — a challenge-response endpoint-ownership handshake before first delivery, and a signed timestamp header behind a versioned signature (breaking for existing consumers — needs a migration note). Delivery through the SSRF guard (7.3.1) and HTTPS-only (7.1.1) are done. (7.1.2, 7.2.3)
