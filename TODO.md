...
- [x] **Webhooks are HTTPS-only** — registration parses the URL into `HttpsUrl` and refuses `http://` with a 400; the dispatcher refuses to dial one and records the failed delivery; pre-existing `http://` subscriptions were disabled by migration 124 (`disabled_reason = needs_https`, shown in the dashboard's org Webhooks card). Plain `http` survives only to loopback when `OVERSLASH_SSRF_ALLOWED_CIDRS` covers it (`services/https_policy.rs`, shared with OIDC discovery). (7.1.1 — now `pass`)
- [x] **Webhook endpoint-ownership handshake** — a new subscription is `pending_verification` until its endpoint echoes a signed `webhook.verification` challenge (2xx, bare or `{"challenge"}`, 10s); registration runs it inline and `POST /v1/webhooks/{id}/verify` re-runs it. Events for a pending subscription are recorded as held deliveries and released on verification; the handshake goes through `ssrf_guard` and the HTTPS rule like every delivery. Migration 125 grandfathered every existing subscription (`verified`, `grandfathered = true`). Dashboard org Webhooks card shows the state and a Verify button. Tests in `tests/webhook_verification.rs`. (7.1.2 — now `pass`)
- [x] **Webhook signed timestamps** — every attempt carries `X-Overslash-Timestamp` and `X-Overslash-Signature-V1: v1=<hmac("<ts>.<body>")>`, re-signed per attempt so a retry gets a fresh timestamp; consumers are told to reject a missing timestamp and anything outside ±5 min (SPEC §webhooks, `sdk` `verifyWebhook`). Not breaking: the legacy body-only `X-Overslash-Signature: sha256=<hex>` is still sent unchanged, so existing verifiers keep working — its removal is tracked in TECH_DEBT.md. Tests in `tests/webhook_signing.rs`. (7.2.3 — now `pass`)
- [ ] **Stop sending the legacy `X-Overslash-Signature`** — see TECH_DEBT.md "Webhooks still send the replayable legacy signature". Also: overslash-docs webhook guide needs the `v1` verification section.
...
- [x] **Dependency vulnerability scanning in CI** — `.github/workflows/deps-audit.yml` runs `cargo-deny` (`deny.toml`), `npm audit --audit-level=high` (dashboard, sdk) and OSV (`osv-scanner.toml`) over every lockfile. `ci.yml` calls it on dependency changes, gated by `ci-ok`, and a daily schedule on `dev` + `master` files a `needs-triage` issue on failure. The four fixable advisories are cleared (`h2`, `rustls`, `crossbeam-epoch`, `event-listener`), plus OSV's `devalue` and three yanked crates. `rsa` and `paste` are time-boxed exceptions with review dates enforced by `scripts/check-deny-reviews.sh`. Policy: [dependency-vulnerability-policy.md](docs/compliance/casa/dependency-vulnerability-policy.md); D101. (6.1.1 — now `pass`)
- [x] **Require TLS on outbound calls** — `services/outbound_tls.rs`. The action transport refuses plain `http` on every hop (redirects included), checked against the address the SSRF guard pinned, and so does the MCP caller; Mode A is refused with or without secrets. The same rule runs as a string check at the boundary — instance `url` on create/update, an org layer's `instance_defaults.url`, a template's `mcp.url` (`endpoint_requires_https` in the validation report) — and again at resolve, so an endpoint stored before this fails with a 400 before any approval is filed. The only way past it is `OVERSLASH_SSRF_ALLOWED_CIDRS`: plain `http` is accepted to an address inside an operator-allowed range, which is how the suite's loopback fakes still work. The dashboard flags an `http://` endpoint next to every base-URL input. Tests in `tests/outbound_tls.rs`. The Cloud SQL `REQUIRE_SSL` and LB SSL-policy halves of 4.1.1 are still open (§1.6 GCP items). (4.1.1 — outbound)
- [x] **TLS on the MCP OAuth upstream hops** — `routes/oauth_upstream.rs`. Resource metadata, AS metadata, dynamic client registration and the token exchange (which sends the authorization code + PKCE verifier and gets back the access and refresh tokens) all go through `outbound_tls::check_resolved` against the SSRF-pinned address, on clients that never follow a redirect. The `token_endpoint` and `authorization_endpoint` the AS metadata advertises are string-checked before anything is registered or a flow minted, so `initiate` fails with a 400 naming the field; the callback re-checks a stored `token_endpoint` before consuming the flow. Same `OVERSLASH_SSRF_ALLOWED_CIDRS` escape hatch. Tests in `tests/oauth_upstream_tls.rs`. (Webhook delivery is HTTPS-only since #664, via `services/https_policy.rs`.) (4.1.1)
...
- [x] **Trusted-proxy configuration for `X-Forwarded-For`**: `services/client_ip.rs`. `ClientIp` now takes the rightmost *untrusted* address instead of the leftmost header value, trusting hops by position (`OVERSLASH_TRUSTED_PROXY_HOPS`), by range (`OVERSLASH_TRUSTED_PROXIES`), and the Vercel rewrite hop by a shared secret its Routing Middleware stamps (`OVERSLASH_TRUSTED_PROXY_SECRET`, `dashboard/middleware.ts`). With nothing configured, the socket peer is the client. Parsed at boot, and a malformed value refuses to start. The two handlers that parsed XFF inline (`/oauth/register`, MCP upstream `initiate`) go through the extractor too. prod/dev values and the enable/rotate/verify steps are in `infra/README.md`. Tests in `tests/client_ip.rs`. **Operator step still open:** set the secret in GSM and Vercel per env, then flip `enable_trusted_proxy_secret`.
