...
- [x] **Declare a credential probe on every template that can have one.** `x-overslash-test` (D-NEXT) is on 20 of the 24. The four without one are deliberate: `deepwiki` (every tool needs a repo name, and it authenticates with nothing), `overslash` itself (`runtime: platform`), and the hidden fixtures `github_legacy_oauth` / `test_email`. Any template added from here should declare one, and `shipped_test_actions_resolve_to_read_actions` holds a floor so the practice cannot quietly lapse.
