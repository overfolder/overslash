...
- **Directory sync follow-ups (D-NEXT).** The OIDC-claim source ships; the rest of the shape is deliberately unbuilt. (a) **Google Admin SDK / Cloud Identity pull** — writes the same tables with `source = 'google_directory'`; needs a stored org-level credential and a sensitive-scope review, and would finally make Workspace groups/aliases work, since Google emits no OIDC group claim at all. (b) **SCIM 2.0 push** (`source = 'scim'`), which is also the natural home for the offboarding half of the `docs/design/user-stories.md` gap. (c) **Periodic re-sync sweep** — today the stale window is a session lifetime; the anchored-claim pattern in `services/webhook_digest.rs` is the model, but it needs a stored directory credential, which the login-time path does not. (d) **General group-in-group nesting** — the `effective_identity_groups` view is deliberately one hop; nesting makes it recursive and needs cycle detection. (e) **ID-token signature verification** — see TECH_DEBT.md.
