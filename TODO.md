...
- [x] **Dependency vulnerability scanning in CI** — `.github/workflows/deps-audit.yml` runs `cargo-deny` (`deny.toml`), `npm audit --audit-level=high` (dashboard, sdk) and OSV (`osv-scanner.toml`) over every lockfile. `ci.yml` calls it on dependency changes, gated by `ci-ok`, and a daily schedule on `dev` + `master` files a `needs-triage` issue on failure. The four fixable advisories are cleared (`h2`, `rustls`, `crossbeam-epoch`, `event-listener`), plus OSV's `devalue` and three yanked crates. `rsa` and `paste` are time-boxed exceptions with review dates enforced by `scripts/check-deny-reviews.sh`. Policy: [dependency-vulnerability-policy.md](docs/compliance/casa/dependency-vulnerability-policy.md); D-NEXT. (6.1.1 — now `pass`)
