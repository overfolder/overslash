...
- [ ] **SSRF on OIDC issuer discovery** — `routes/org_idp_configs.rs:135` and `:521` dial an admin-supplied `issuer_url` through the shared `state.http_client`, behind a hand-rolled string check (`services/oidc_discovery.rs:85-125`) instead of `ssrf_guard`. That check never resolves DNS, so a hostname pointing at 10.0.0.1 passes, and the shared client follows redirects. The handler echoes the fetch error and body, making it an oracle. Remedy: delete the hand-rolled check, call `ssrf_guard::outbound_client`. Org-admin only, so narrower than V1. (CASA 5.1.5)
