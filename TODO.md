...
- [ ] **Declare a credential probe on the remaining templates.** `x-overslash-test` (D-NEXT) is on 18 of them; `deepwiki` genuinely has none (every tool needs a repo name, and it authenticates with nothing). Any template added from here should declare one, and `shipped_test_actions_resolve_to_read_actions` holds a floor so the practice cannot quietly lapse.
