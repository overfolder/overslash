...
## `SCHEMA.sql` drifts from the migrations

`SCHEMA.sql` is a convenience dump (`make schema`), regenerated by hand and
verified by nobody — no CI job compares it against a freshly migrated database.
As of migration 115 it is ~230 lines behind: it is missing whole columns
(`orgs.require_invite_admission` and `orgs.managed_signin_allowed_domains`,
added in 092), several `COMMENT ON COLUMN`s, and it still cites D56 where the
comments now say D59. Reading it as the schema of record will mislead you.

Migration 115 patched only the `identities` indexes it changes rather than
regenerating the file, to keep a 4-line schema change from arriving as a
