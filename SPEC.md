...
- **`x-overslash-sql-field` / `sql-field:`** — on a parameter, a dotted body path that both nominates this param as the raw-SQL field (D42 content policy: parse → read/write risk floor, per-table permission keys `{service}:{action}:table={label}/{relation}` + all-tables sentinel, `column=`/`column_star=` deny screening) and names where the SQL string sits in the assembled JSON body. On a **string** param the value is *placed* at the path (`native.query` keeps the caller surface flat while nesting the outgoing body); on an **object** param the path is *descended into* (it must anchor at the param's own name). One per action, enforced at template compile.
