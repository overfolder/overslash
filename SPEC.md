...
**Layer-2 seed for first-level agents (D79).** That Everyone→`overslash:write` grant means Layer 1 has always permitted an agent's own setup calls; only Layer 2 refused them, because nothing seeded a rule for a new identity. A newly-created agent at `depth = 1` is therefore seeded with four allow rules — `overslash:{manage_services_own,manage_templates_own,manage_connections_own,request_secrets_own}:*` — so it can stand up the services it needs without an approval per call. Gated by `orgs.default_agent_self_setup` (default `true`), applied at creation time only, and never widened to the `_share`/`_publish` half of any split. Sub-agents are not seeded. The rules are ordinary `permission_rules` rows, revocable per agent. Agents that predate the flag are caught up only when an admin asks: `POST /v1/orgs/{id}/agent-self-setup/backfill` grants the same four anchors to every live first-level agent missing them, and refuses while the flag is off.
...
- **`x-overslash-scope_param` / `scope_param:`** — which parameter(s) provide the `{arg}` segment in permission keys. Without it, the arg is `*`. Accepts a param name (`scope_param: repo`), a `param:label` pair, or a list of either (`scope_param: [to:recipient, cc:recipient, bcc:recipient]`). Each value mints one `{service}:{action}:{label}={value}` key — an array-valued param fans out per element — and the label defaults to the param name. Keys are deduped, so params sharing a label collapse a value that appears in more than one of them into a single key (and a single approval). See §5 for how rules match labelled keys. A list entry may also be a **mapping** — `{param, label?, extract}` — where `extract` is a jq program run over **that param's own value**, for a param whose value is not itself the thing being gated: Graph's recipients are `[{emailAddress: {address}}]`, so `extract: '.toRecipients[]?, .ccRecipients[]? | .emailAddress.address'` mints one key per address instead of filing the whole JSON object as the key's value. The two spellings mix freely in one list. An extractor is rooted at its param, never at the whole params map, so a key's provenance stays one param and `unknown_scope_param` keeps meaning something; `extract` and a resolver's `scope:` on the same param are mutually exclusive, since `scope` collapses the value to one canonical string the extractor was not written to read. Extracted values are sanitized (`=`, `/`, braces and commas collapse to `-`) because they are content lifted out of caller-supplied structure rather than a caller-typed argument. **Failure is narrowing, never widening**: an `extract` that errors, times out, yields a non-scalar or empty output, or exceeds 64 values mints `{service}:{action}:scope_error={label}/{param}` *and suppresses the `:*` fallback*, so the call raises an approval rather than being gated on the entries that happened to resolve. That sentinel has its own label and a `/` in its value, so neither a `{label}=*` grant nor a plain `{service}:{action}:*` covers it — only the honest `{service}:{action}:**`. It is deliberately **approvable but not rememberable**: no broadening rung is offered for it, a `remember_keys` entry whose only coverage is a sentinel is refused, and a bare Allow & Remember degrades to a one-shot allow, because a standing rule covering "we could not tell" would authorize every future call that fails the same way.
...
- **`x-overslash-sql-database` / `sql-database:`** — a jq expression over the call params (e.g. `.database | tostring`) whose result keys into the instance's `sql_databases` config map (`{"<db-key>": {"dialect": "...", "label": "...", "safe_functions": [...]}}`) to resolve the parse dialect and (D69) any extension or in-house functions this database vouches for on top of the shipped `pg_catalog` safe list. Unresolved falls back to postgres with no extra functions (fail-closed). The **DB label** used in audit rows, tags and permission keys is the database's own name, taken from the `scope:` of an `x-overslash-resolve` declared on this same param; `sql_databases[key].label` overrides it, and the raw key is the fallback when neither answers. Whichever name wins, the raw key rides along as the key's second spelling — `table={label,key}/{relation}` matches a rule written on either, so an upstream rename cannot orphan a grant and a resolver that does not answer degrades the key rather than re-targeting it. Tags carry the pair as `db:` + `db_id:`; the audit `detail.sql` block as `db` + `db_id`.
...
- **`x-overslash-default_secret_name` / `default_secret_name:`** — on an `apiKey` or `http` security scheme, the canonical secret name for auto-wiring. Within a single auth mode there is no fallback between kinds: a mode backed by OAuth never falls back to a secret, and a mode backed by a secret never borrows a connection. A template that wants to offer both declares them as alternative **auth modes** rather than as one mode holding both — see `x-overslash-auth-modes`.
- **`x-overslash-auth-modes`** — under `components`, the alternative credential kinds this template accepts, as a map of mode key to `{label, description, schemes, default}`. Prefixed spelling only, like its `components` siblings `x-overslash-secrets` and `x-overslash-config`. `schemes` names the `securitySchemes` entries the mode activates; `label` is what the dashboard's picker renders; exactly one mode carries `default: true` once more than one is declared. Figma, GitHub and Notion each accept an OAuth token *or* a long-lived token against the same host and the same paths, and this is how a template says so — one template with two modes, rather than the second template file that used to be the only way to express it.
  **Absent means no alternation**, which is the reading every template had before this existed: every declared scheme applies to every request, together. That is what `services/email.yaml` depends on, where the org's gateway key and the per-mailbox login are both required on the same call. So a template declaring no modes has exactly one implicit mode holding all of its schemes, and nothing about it changes.
  **The choice is made per instance, at creation, and stored** (`service_instances.auth_mode`, `auth_mode` on `POST /v1/services`; omitted resolves to the default, an unknown key is a `400` naming the valid ones). It is deliberately not inferred from whichever credential happens to be bound, because the state that has to be answered is a *freshly created* instance with neither bound yet: that is exactly when the credentials badge must say which credential is still owed, and when `create_service` must decide whether to return `connect.auth_url` or `setup.setup_url`. Inference has no answer there, and an instance that guessed wrong is one whose probe can never pass.
  Everything downstream reads the template **through** the mode: which slots the instance owes, whether auto-connect fires, whether the default-connection fallback applies, and which credential the executor injects. A scheme may belong to several modes, which is how a credential shared by every alternative rides along with whichever one was picked. A scheme belonging to *no* mode is a template error — the operator could fill it and no request would ever carry it.
  **Switching is supported and destroys nothing**: `PUT /v1/services/{id}/manage` accepts a new `auth_mode`, keeps both the bound credentials and the connection, drops the instance back to `pending_setup`, and re-mints the matching handshake — the instance goes live again only when the probe passes (D86).
  `security:` is **not** the alternation, although OpenAPI's requirement-object array does mean OR. In this gateway `security:` is read purely as the *scopes* an OAuth connection needs, and a requirement object has nowhere to carry a stable key, a display label or a description — all three of which the picker and the stored column need.
...
- **`x-overslash-icon` / `icon:`** — under `info`, the mark the dashboard shows for this service. Two forms: `builtin:<name>`, an asset Overslash ships and serves at `/icons/<name>.svg`, and an `https://` URL hosted elsewhere. **Usually omitted**: a template whose key matches a shipped asset resolves to `builtin:<key>` implicitly, which is why the shipped templates declare nothing. Explicit values are for the two cases the convention can't express — a key that deliberately differs from the asset it reuses, or a remote URL. Resolved server-side and surfaced as an absolute `icon_url` on the template and service-instance responses; a template with nothing renderable omits the field, and the dashboard falls back to a letter tile. Only `https://` ever reaches a browser — `http:`, `data:` and `javascript:` are template *errors*, checked both at write time and again when the response is built. Overslash never fetches a remote icon (that would be an SSRF vector and a boot-time network dependency), so there is no size or format validation of one. `${VAR}` expansion applies like anywhere else, which is how a self-hoster points the set at their own CDN. Icons are deliberately **not** on `/v1/search`: it fans out up to 100 rows per (instance × action) and the field would cost an agent's context window for something no model can render.
- **`x-overslash-additional-properties` / `additional-properties:`** — on an operation or MCP tool, that the action's declared parameter set is a **floor, not a fence**: an argument the template never declared is forwarded upstream instead of returning `invalid_action_args`, and a declared `enum` names the members we know of rather than all of them. `required` is still enforced either way — it is a statement about the operation, not about how completely we transcribed it. Also settable under `info` as a service-wide default, where the nearest value wins, so an operation may write `false` to re-tighten under a service that relaxed globally. **Defaults to `false`**, and a malformed value (`additional-properties: "true"`) warns and stays strict, so a quoting mistake cannot grant the relaxation by accident. The name is borrowed from JSON Schema, where it governs undeclared keys only; here it deliberately covers `enum` too, because the same template that cannot name every parameter is the one whose enum snapshot has gone stale. For a relaxed action whose operation declares no `requestBody`, a non-`GET` call synthesizes a JSON body so the accepted argument actually reaches the wire; declaring the key on a body-carrying method whose `requestBody` is **non-JSON** is a template *error* (`additional_properties_needs_a_json_body`), since only JSON bodies are serialised and the argument would otherwise pass validation and then be dropped. **Not read on a platform action**: those answer from this process against a param set Overslash owns, so an undeclared argument would pass the gate and then be silently dropped — strictly worse than the 400. What the author gives up is the Levenshtein "did you mean" on a typo: a misspelled optional param now reaches the upstream instead of returning a self-correctable 400. Two security notes, both consequences of the gateway not knowing a name exists: permission-key derivation (`x-overslash-scope_param`) reads declared params only, so an undeclared argument can change what a call does without changing the key the chain checks; and `x-overslash-redact` cannot cover a path it has never seen, so a relaxed action's approval payload and audit row may carry undeclared values in the clear. Surfaced to agents as `additional_properties` on `/v1/search` rows, and to the dashboard on the action-detail response, which is what turns the API Explorer's enum `<select>` into an open combobox and adds its free-form argument rows.
- **Nested parameter schemas** — a parameter's `properties` / `items` are read, so an `object` or `array` param states its shape instead of describing it in prose. Carried as a reduced JSON Schema — type, `required`, `description`, `enum`, `default` and the recursion — because nothing reads `format`, `minimum` or `oneOf`, the same reason the flat parameter contract carries no more than it does. Lowering stops at 5 levels and does not follow `$ref`; a level past the cap, an `items: {}`, or an object with no `properties` keeps the bare `object`/`array` the param already had. At call time the shape is enforced with **exactly the three checks the top level performs** — required present, unknown key rejected, string `enum` respected — at every depth, naming the field by its full path (`createRequest.objects[0].objectType`). Type rejection stays out of scope at every depth, so a value whose *kind* does not match is not an error and is simply not descended into: the shape describes a structure, it does not assert one. `x-overslash-additional-properties` relaxes the nested checks exactly as it relaxes the flat ones, and a sub-schema may open itself further with JSON Schema's own `additionalProperties: true` (read as a bool *or* as a schema) but cannot re-close a level its action opened. An array is checked to its first 100 elements. The shape reaches agents on `/v1/search` rows, capped at 3 levels and 24 nested nodes per parameter with a `truncated` flag when either bites, and reaches the dashboard uncapped, where the API Explorer renders it as a skeleton placeholder, a field list and live validation.
- **`contentMediaType` / `contentSchema`** — on a `string` param, that its wire value is a *serialized* document rather than the document itself (`services/telegram.yaml`'s `invoke_mtproto.params_json`). Only `application/json` is honoured, and only on a string; `contentSchema` without it is a template error, since it would describe a document nothing parses. A caller may pass either the structure or the string: structure is serialized **once**, during argument coercion — above validation, permission-key derivation, the approval and the replay payload, which is what makes "the bytes a human approved are the bytes later replayed" true rather than hoped for — and a string the caller supplied is **never re-encoded**, because `serde_json` re-sorts object keys on a round trip and the gateway must not hand the upstream different bytes than it was given. Validation parses into a read-only view; a string that is not JSON is `invalid_action_args` even under `additional-properties`, because that relaxation exists for stale *enums* and "is this string JSON" cannot go stale. A param declaring no `contentSchema` is never parsed for a shape at all. `x-overslash-redact` may address a path *inside* such a param — the string is parsed, redacted and re-serialized in the display projection — which the other dotted-path surfaces deliberately do not do; a `scope_param` on one requires an `extract` (write the `fromjson` yourself), since the whole literal is not a permission key anyone can match or read.
...
Both credential kinds are returned by the *same* call, on the same response, as sibling fields: `connect` (`auth_url`, `state`, `flow_id`) for an OAuth-backed template, and `setup` (`setup_url`, `short_url`, `requests[]`) for a secret-backed one — one entry per unbound per-instance credential slot. `skip_connect` and `skip_credentials` suppress the respective half for callers that intend to wire credentials themselves. The URL is a bearer capability and is returned only to the caller that minted it: it never appears on an event payload or in an audit detail.

### Test actions

A template may nominate exactly one action as its **credential probe** — the read it runs to prove an instance's credentials work — with `x-overslash-test` (unprefixed alias `test:`), valued `true` or `{ params: { … } }` on an operation or an MCP tool. `POST /v1/services/{id}/test` resolves that action and runs it, so no caller has to know which one it is, and answers with a verdict: `ok`, `failed`, `denied` (a permission rule refuses the call), `pending_approval`, `needs_authentication`, or `not_supported` (the template declares none). The verdict carries the upstream status, wall-clock latency, the call's `action_description`, and a truncated upstream error — never the response body.

The probe is **not** an approval bypass. It runs through the ordinary `/v1/actions/call` path, so the permission chain and the Layer-2 gate both apply; `pending_approval` is a real verdict. In the case it exists for this is invisible: a fresh instance's Myself auto-grant carries `auto_approve_level = 'read'` and the probe is forced to `risk: read`.

Template validation makes two rules hard errors, because the probe fires unattended the moment a credential lands: at most one action per template may carry `x-overslash-test`, and it must be `risk: read`. Its `params` must name real params of the action and cover every required one that declares no `default`.

**A service does not go live until its probe passes.** An instance created by a setup flow is written `status = 'pending_setup'`: it exists, it can be edited and probed, and it is **not callable** — it resolves by name nowhere and appears in no search result. `POST /v1/services/{id}/activate` runs the same probe and promotes it to `active` on `ok` (or on `not_supported`, since a template can lose its probe after an instance was gated on it). Every outcome is a `200`, a red verdict included: "your credential does not work" is the answer to the question, not a failure to answer it. `?force=true` is the audited override — it runs no probe and returns no verdict — as is `PATCH /v1/services/{id}/status`, which now records `bypassed_verification`.

Promotion is deliberately *not* a side effect of `/test`: that endpoint is a diagnostic an org admin may run against instances they do not own, and promote-on-green there would silently publish other people's unverified drafts.

The gate defaults on exactly where a probe runner exists — a template that declares a probe, a per-instance slot nobody bound, and an owner, i.e. precisely when `create_service` mints a setup link and a signed-in human will land on the setup page. It is left off for an OAuth create (the callback is server-side, with no caller to probe as), for a create whose credentials are already bound, for `skip_credentials`, and for an org-level instance. `verify: true` forces it on for a caller that probes itself, and is a `400` on a template that declares no probe; `verify: false` always creates live. Note that an agent cannot lift the gate itself: `/activate` is owner-or-admin and an agent is not an ancestor of its own owner-user.

`pending_setup` is never a valid target of `PATCH /status` — only creation enters it, and only activation (or the override) leaves it. That asymmetry is what lets the background sweeper delete every such row on age alone, after `MAX_LINK_TTL_SECS + sweep_grace` (~24h), measured from `created_at`. The purge cascades the instance's outstanding setup links and leaves the vault secret standing.
...
- **Service setup** (`/services/setup/req_...?token=jwt`) — the service-shaped sibling of the page above, and the same handshake: same signed token, same single-use row, same User Signed Mode rules, and it submits to the same `POST /public/secrets/provide/{req_id}` endpoint. What differs is the framing and what fulfilment does. The page leads with the service (icon, display name, instance name, the slot's authored label and help text) rather than with a vault key name, and on submit the server binds the instance's credential slot as well as writing the vault secret — so the instance goes from credential-less to callable in one POST. A request with no `service_instance_id` 404s here and keeps the older page. A setup link **always** requires a same-org session, whatever the org's `allow_unsigned_secret_provide` says — fulfilling one triggers the credential probe, the probe runs through the authenticated call path, and an anonymous fulfilment could therefore never produce a verdict, leaving the instance accepted-but-never-live. On submit the page runs the probe (§9 *Test actions*) and, on a green verdict, the instance goes live; a visitor who is not the instance's owner gets `denied` and the service stays awaiting setup for its owner to finish.

  These URLs are minted by `POST /v1/services` for every unbound per-instance credential slot and returned as `setup.setup_url` — the secret-path twin of the `connect.auth_url` an OAuth-backed template already gets. `skip_credentials: true` suppresses them. `POST /v1/secrets/requests` and the `request_secret` platform action also accept `service_id` / `credential_key` for binding a credential to an instance that already exists.
...
### `link`: the upstream names the next URL

The other three styles name a *value* the next call carries. `link` is for an
upstream that hands back a whole URL instead, and the gateway decomposes it
rather than adopting it: only the query keys the action already declares are
lifted out, so the continuation stays inside the action's own contract instead
of becoming a second way to address the upstream.

Two places a response can carry that URL, and the declaration picks one:

```yaml
# GitHub — RFC 8288 Link header (the default; `from` absent)
next: { style: link }

# Shortcut — a whole URL path + query string in the body
next: { style: link, from: next, param: next }
```
...
name a parameter the action declares.
...
An MCP tool may declare `link` only with `from` — a JSON-RPC result has no
headers — and the body path resolves against the tool's own payload under the
same projection `resolve:` uses (D78).
...
- **Health vs. readiness**: `/health` is liveness — it reports database reachability in the body (`db`, plus `db_latency_ms` when up; the error itself is logged, never returned, since both endpoints are unauthenticated) but always returns 200, because it backs the Cloud Run startup and liveness probes and a 503 there would restart containers during a database outage. `/ready` runs the same bounded `SELECT 1` and returns **503** when Postgres is unreachable; it is the endpoint to point a load balancer or alerting monitor at.
