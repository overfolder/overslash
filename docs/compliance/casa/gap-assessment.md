...
| 1.2.1 | No default credentials on publicly exposed interfaces | `pass` | No default accounts ship, and no default *key* can start a deployment. `.env.example` no longer carries working values for `SECRETS_ENCRYPTION_KEY` / `SIGNING_KEY` (it says `openssl rand -hex 32`), and the boot interlock `Config::boot_policy` (`config/boot_policy.rs`, enforced by `overslash-cli/src/common.rs::load_config`) refuses a placeholder, one repeated byte, fewer than 32 bytes, or fewer than 8 distinct byte values — every value `.env.example` and the test fixtures ever shipped — whenever `OVERSLASH_ENV` is anything but unset/`local`. The container image sets `OVERSLASH_ENV=prod` (`crates/overslash-api/Dockerfile`), so a copied `.env.example` cannot start the shipped artifact; a local source checkout only warns, so a developer database keeps decrypting. `SECRETS_ENCRYPTION_KEY_PREVIOUS` is exempt on purpose, so the remediation — rotating off a weak key — stays possible. The hardcoded `Keyring::test()` key is compiled only under `cfg(test)` / the dev-dependency-only `test-support` feature (`overslash-core/src/crypto.rs`). Tests: `config::boot_policy::tests` | — |
...
| 6.2.1 | Debug modes disabled in production | `pass` | Four interlocks, all in `config/boot_policy.rs` over a parsed `DeploymentEnv` (`OVERSLASH_ENV`; the container image defaults it to `prod`). (a) `DEV_AUTH` is read with the shared truthiness rule (`overslash_env::parse_bool`) — `0`, `false` and `""` are off — and any value that is not a recognised boolean now **stops the boot** rather than silently picking a side; `DEV_AUTH` on with `OVERSLASH_ENV=prod` stops the boot (`BootViolation::DevAuthInProduction`), so `/auth/dev/token` and `routes/dev_e2e.rs` cannot be live in production. (b) `RUST_LOG` defaults to `info`, and in production a filter enabling `debug`/`trace` anywhere is replaced by `info` with a warning (`log_filter`, wired in `overslash-cli/src/common.rs::init_tracing`) — clamped rather than refused so a verbosity change cannot crash-loop the service; `infra/env/prod.tfvars` now sets `rust_log = "info"`. (c) `/health` and `/ready` no longer echo the sqlx error — the body carries only `"db": "down"` and the error goes to the log (`routes/health.rs`). (d) Weak keys refused outside local (1.2.1). Tests: `config::boot_policy::tests`, `routes::health::tests`, `tests/health.rs`, `overslash-env/tests/env.rs` | — |
...
Of the 6 gaps, **none is a live vulnerability** any more. The counts moved from the original assessment because the two
...
made 7.2.3 `pass`. The boot interlocks (DEV_AUTH, weak keys, production log level,
`/health` error text) made 6.2.1 and 1.2.1 `pass`.
...
`RUST_LOG` off `debug` in production (6.2.1, 1.2.1)~~ — done; rate limiting extended to
