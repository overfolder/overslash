...
Rows about infrastructure were written from the Terraform in `infra/`. Where a live
`gcloud` scan has since confirmed or extended them, the row links to
[gcp-posture.md](gcp-posture.md) — which also carries two findings the IaC could not
show, because they are GCP defaults Terraform never created.
...
| Req | Requirement | Verdict | Evidence / gap | Sev |
|-----|-------------|---------|----------------|-----|
| 3.1.1 | Least-privilege access control on a trusted service layer | `pass` | Capability types in `crates/overslash-db/src/scopes/` (`SystemScope > OrgScope > UserScope > AgentScope`) inject `org_id` into every query. `/v1/actions/call` runs two layers: a default-deny group ceiling (`overslash-core/src/permissions/ceiling.rs`, enforced at `routes/actions/call.rs:346-356`) and an ancestor-chain walk with approval bubbling (`routes/actions/permission_gate.rs:48-90`). Enforcement is server-side only. **Infrastructure caveat from the live scan:** the default Compute Engine service account holds `roles/editor` on the project — a GCP default that is invisible to Terraform and that Recommender flags at P2 as unused — [gcp-posture.md](gcp-posture.md) | — |
