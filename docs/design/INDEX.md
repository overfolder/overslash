...
| [white-label-token-vault.md](white-label-token-vault.md) | Approved — implemented | White-label OAuth as a token vault — the partner runs the OAuth dance and `POST /v1/connections/import`s the tokens; overslash stores + refreshes (self-refresh via BYOC, or integration-managed when no client is shared) + executes. Reverts the per-request `redirect_uri` (#388/#392) and per-org `oauth_redirect_url` (#398); removes `/v1/oauth/exchange`, `include_raw`, and `oauth_connection_flows.redirect_uri`. |
