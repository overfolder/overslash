...
| [mcp-elicitation-approvals.md](mcp-elicitation-approvals.md) | Draft | Map Overslash approvals onto MCP `elicitation/create` (form + URL mode) and the experimental `tasks` primitive — sync flow works on Claude Code 2.1.76 / Codex v2 today, async via task-augmented `tools/call` is the end-state |
