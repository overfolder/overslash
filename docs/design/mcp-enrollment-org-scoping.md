...
3. **Scope the fast-path binding lookup by the resolved org.** Resolve `(user, client_id, org) → agent`. A binding from another org cannot satisfy an authorize on this subdomain. **This requires a schema change:** `mcp_client_agent_bindings` currently has `UNIQUE (user_identity_id, client_id)` — one binding per `(user, client)` *globally* — which makes a per-org binding impossible. Migrate it to **`UNIQUE (user_identity_id, client_id, org_id)`** so the same user+client can bind a distinct agent per org. This is not merely defensive: the root multi-org case (below) has a single `NULL`-scoped root client that a member may enroll into *two* orgs they belong to; without `org_id` in the key the second enrollment collides with the first.

   The constraint change forces matching code edits in `crates/overslash-db/src/repos/mcp_client_agent_binding.rs` (all keyed on the old pair today): (a) **`upsert`** — its `ON CONFLICT (user_identity_id, client_id)` becomes `ON CONFLICT (user_identity_id, client_id, org_id)`, else the upsert either errors or clobbers a sibling org's binding; (b) **`get_for`** — gains an `org_id` parameter and a `WHERE … AND org_id = $3`, else it is non-deterministic across a user's orgs (its callers in `authorize` pass the resolved org); (c) the module-level doc comment that asserts the old `(user_identity_id, client_id)` uniqueness is updated.
