...
- `credential_missing { service?, secret_name, self_serve, hint_url? }` — a non-OAuth secret the action needs is absent. `self_serve` names the `request_secret` call that mints a provide URL for `secret_name`; `hint_url` is the dashboard page a person would open. Both ship because they address different readers — a dashboard link is inert to an agent, and an agent that can mint the link itself only needs the human to paste a value. The secret-backed `needs_authentication` shape carries the same field, one entry per `missing_credentials` slot. The OAuth shapes do not: `auth_url` already answers them, and minting a second flow would waste a consent round-trip.
...
1. **Overslash permissions** — the agent's identity must hold the relevant `manage_services_own`, `self_approve`, etc. scope. Granted by a human: either explicitly in the dashboard, or — for the four `_own` anchors on a first-level agent — seeded at the moment a human creates that agent, if the org leaves `default_agent_self_setup` on. Everything outside those four, `self_approve` included, is still an explicit dashboard act.
