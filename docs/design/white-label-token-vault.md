...
The same "no overslash reconnect URL" rule applies to the **`missing_scopes`** envelope: when an action needs a scope a *known-but-insufficient* integration-managed connection lacks, `check_required_scopes` skips the upgrade-URL mint (Overslash has no client, and minting would leave a stray flow row) and returns `missing_scopes` with `auth_url` omitted — the integration broadens the grant and re-imports.
